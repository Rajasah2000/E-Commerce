...
export const BASE_URL = 'http://13.127.222.180:4051/v1/admin/';
...
async function OcrFileUplode(url = 'POST', method, file, object_get = {}, tokenCustom = null) {
  let token = '';
  let user = reactLocalStorage.getObject('userData');
  // if (user && user != null && Object.keys(user).length > 0) {
  //   token = user.token;
  // }
  // let bash_url = "http://13.127.101.102:5011/api/v1/";
  let apiUrl = 'http://ocr.asprise.com/api/v1/receipt';
  // let data = new FormData();
  // data.append("image", file);
  // Object.keys(object_get).forEach(function (key) {
  //   let ddd = object_get[key];
